...
void push_front(shared_ptr<list_t>& list, shared_ptr<node_t>& node){
    if (!(list || node)) cout << "nullptr entered"; LOG_FUNCTION_NAME();return;
    node->next = list->head;
    list->head = node;
...
void push_back(shared_ptr<list_t>& list, shared_ptr<node_t>& node){
    ...
    list->tail->next = node;
    list->tail = node;
    if (list->size == 0) list->head = list->tail;
    list->size++;
}
...
void erase(shared_ptr<list_t>& list, int position){
    if (!list) cout << "nullptr entered" ; LOG_FUNCTION_NAME(); return;
    if (list->size == 0) cout << "Cant erase from empty list"; LOG_FUNCTION_NAME(); return;
    if (position > list->size-1) {
...
void print_list(shared_ptr<list_t>& list){
    if (!list) cout << "nullptr entered" ; LOG_FUNCTION_NAME(); return;
    if (list->size == 0) cout << "Cant print an empty list"; LOG_FUNCTION_NAME(); return;
    auto current = list->head;
    while (current) cout << current->value << "->" << endl;
}
